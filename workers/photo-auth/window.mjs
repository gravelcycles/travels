...
let leaving=false,activeController;
...
const send=payload=>{if(leaving)return;leaving=true;const target=new URL(returnTo);target.hash=new URLSearchParams({state,...(payload.loggedOut?{photoAuthLogout:'1'}:payload.missing?{photoAuthMissing:'1'}:{photoAuthCode:payload.code})}).toString();location.replace(target.href);};
done.onclick=()=>{leaving=true;activeController?.abort();const target=new URL(returnTo);target.hash=new URLSearchParams({state,photoAuthCancel:'1'}).toString();location.replace(target.href);};
...
async function start(){
  retry.hidden=true;form.hidden=true;message.textContent=logout?'Locking photos…':'Restoring photo access…';
  try{if(logout){await request('logout',{});send({loggedOut:true});return;}send(await request('session',{}));}
    ...
    if(!logout){document.querySelector('#intro').hidden=false;form.hidden=false;input.focus();}
  }
}
retry.onclick=start;
