...
export function photoWidths(width, height) {
  ...
  return [...new Set([Math.min(1280,big),big])];
}
...
export async function buildPhotoVariants(root, image, width, height) {
  ...
  for (const size of photoWidths(width,height)) {
    const { data, info } = await image.clone().resize({width:size,withoutEnlargement:true}).webp({quality:84,effort:5,smartSubsample:true}).toBuffer({resolveWithObject:true});
    variants.push({src:storeDerivative(root,data),width:info.width,height:info.height,bytes:info.size});
  }
  return variants;
}
export function existingTwoVariants(photo) {
  ...
  const big=candidates.at(-1),small=candidates.find(v=>v.width>=1280)||big;
  return small.src===big.src?[big]:[small,big];
}
