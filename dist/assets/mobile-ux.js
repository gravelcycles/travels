(function (root) {
  ...
  function create(api) {
    ...
    function update(data) {
      ...
      $('#mobile-photo-time').textContent = photo?.mediaType === 'video' ? photo.title || photo.caption : photo?.takenAt || data.day.date;
    ...
    function pointerDown(event) {
      ...
      suppressClick = false;
      if ((event.target.closest('button:not(#mobile-photo-location)') || event.target.closest('#viewer-video'))) return;
      stopSwipe(); stopZoom(); clearTimeout(locationTimer); viewer.classList.remove('is-location-settling');
      pointers.set(event.pointerId, {x:event.clientX,y:event.clientY});
      event.currentTarget.setPointerCapture(event.pointerId);
      if (pointers.size === 2) {
        if (current.photos[current.index]?.mediaType === "video") return;
    ...
    stage.addEventListener('click', event => {
      if (!enabled() || (event.target.closest('button') || event.target.closest('#viewer-video')) || current.photos[current.index]?.mediaType === 'video' || grid) return;
    ...
    media.addEventListener('change',()=>{
      ...
      if(replayState)replayControlsChanged(replayState);
      renderDay();save();
    });
