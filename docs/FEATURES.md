...
| Curated places, food and group reviews | Shared Places map layer/panel, food/sight and day filters, source links, credited image carousel, group average and individual notes, saved/unvisited state; same controls on phone and desktop | Optional validated `pointsOfInterest`; absent data shows an empty state. Prompt-authored public content, separate from itinerary route nodes | places-comments.js/css, places-content.mjs, journey.html |
