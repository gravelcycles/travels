...
| Photo comments UX preview | Display name + sample password; persistent local identity; name changes without password; per-photo draft recovery; photo navigation, edit/delete-own with Undo, validation and saved/error states; keyboard, short-screen and nested Back/Forward support | `experience=comments` or `experience=places`; browser-only writes. Sample password `demo` never unlocks private photos. Shared Worker/database writes remain proposed in [PLACES_AND_COMMENTS.md](PLACES_AND_COMMENTS.md) | photo-comments.js/css, places-comments.js/css, mobile-ux.js, journey.html |
