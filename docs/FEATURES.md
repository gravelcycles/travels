...
| Route overview and focus | Whole-trip/day bounds, optional wider geographic context for the overview, quiet context, compact charcoal city/town/village names with opaque warm-white halos above routes; road labels and highway shields below routes, route inspection and named stops; map credits start collapsed behind a working info button in every map; quiet intermediate rail stops with small orange centers and broad white rims spanning the blue line and overlapping its casing; larger 12 px start/end dots at both ends of every train leg. Stop dots appear only once their train lines render, with pending reveals cancelled on day changes. Small upright numbered signposts identify single-day destinations; repeated/nearby stops use quiet dots with explicit day choices. Pins cluster on zoom, keep numbered boards above the route on short vertical stems, and protect complete tap targets from controls. Posts never rotate or flip; their feet mark actual locations, with a dot fallback when space is tight. Hover/focus links pins, complete day routes and day rows, with one compact place card and no camera movement. Single-day click/tap opens the day; phone targets are 44 px, and accessible labels expose place/day information | Places, ordered `segmentIds`, segments/geometry; optional `overviewBounds` expands whole-trip framing, otherwise route/place bounds apply; map actions disable when no coordinates exist | app.js, location-labels.js |
| Curated places, food and group reviews | Shared Places map layer with Google Maps-style photo tiles and a left detail panel, Overview/Reviews/Photos tabs, circular quick actions, star distribution, food/sight and day filters, credited photos, group averages and individual notes; expandable phone sheet and saved/unvisited state | Optional validated `pointsOfInterest`; absent data shows an empty state. Prompt-authored public content, separate from itinerary route nodes | places-comments.js/css, places-content.mjs, journey.html |
| Photo comments UX preview | `experience=comments` demonstrates display name + sample password, photo-linked plain-text comments and own-comment deletion; `experience=places` also enables a local group-review editor | Browser-only demo writes; sample password `demo` never unlocks private photos. Shared Worker/database writes remain proposed in [PLACES_AND_COMMENTS.md](PLACES_AND_COMMENTS.md) | places-comments.js/css, journey.html |
