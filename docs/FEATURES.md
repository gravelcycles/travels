...
| Photo loading/access | Responsive thumbnails, full-size viewer readiness, desktop blur-to-focus reveals on every appearance (including cached photos and video posters), reduced-motion support, bounded preloads, shared password access and remembered login | Publish reviewed derivatives into a journey-specific private namespace; no new login implementation per trip | photo-auth.js, Worker, photo scripts |
