...
async function runWindow(action,status){
 ...
 const getNode=selector=>{if(!nodes.has(selector))nodes.set(selector,{hidden:['main','#login','#intro','#done'].includes(selector),textContent:'',focus(){this.focused=true;}});return nodes.get(selector);};
...
test('remembered sessions return automatically without revealing the password form',async()=>{
 ...
 assert.equal(f.nodes.get('#login').hidden,true);assert.equal(f.nodes.get('main')?.hidden??true,true);assert.equal(f.redirects.length,1);
